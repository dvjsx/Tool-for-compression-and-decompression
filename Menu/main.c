		...
		case KEY_ENTER:case KEY_RENTER:
			if (highlight == MENU_OPTIONS - 1) exit(0);
				...
				if (fp.size) {
					...
					out = fopen((!highlight) ? "to_compress.txt" : "to_decompress.txt", "w");
					assert(out);
					...
					if (highlight == 0) callCompression(fp, log, algo);
					else if (highlight == 1) callDecompression(fp, log);
...
void startCMD(int argc, char *argv[]) {
	...
	if (argc == 2) {
		...
		if (fp.size > 0) {
			FILE *out = fopen("cmd_output.txt", "w");
			for (int i = 0; i < fp.size; i++) {
				fprintf(out, "%s\n", fp.paths[i]);
			}
			fclose(out);
